  ...
  return (
    <Section id="parents" className="bg-cream">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-16 items-start">
        <Reveal>
          <h3 className="font-fredoka text-[27px] font-bold mb-8">Как мы работаем</h3>
          <div className="flex flex-col">
            {[ 
              ...
              <div key={i} className={`flex gap-5 relative ${i < arr.length - 1 ? 'pb-8' : ''}`}>
                ...
                <div className="w-12 h-12 rounded-full bg-em text-white font-fredoka text-[21px] font-bold flex items-center justify-center shrink-0 relative z-10 shadow-[0_4px_16px_rgba(15,139,109,.3)]">
                  {i + 1}
                </div>
                <div className="pt-2">
                  <h3 className="text-[18px] font-extrabold mb-1.5">{step.title}</h3>
                  <p className="text-[15px] text-tx-m leading-[1.6]">{step.desc}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <Reveal delay={0.2}>
          <h3 className="font-fredoka text-[27px] font-bold mb-8">Частые вопросы</h3>
          <div className="flex flex-col gap-3">
            {FAQS.map((faq, i) => (
              <div key={i} className="bg-white rounded-[14px] overflow-hidden shadow-card">
                <button
                  onClick={() => setOpenFaq(openFaq === i ? null : i)}
                  className="w-full text-left px-5 py-4 text-[16px] font-bold flex justify-between items-center transition-colors hover:text-em"
                >
                  <span className="pr-4">{faq.q}</span>
                  <span className={`text-[19px] text-em transition-transform duration-300 ${openFaq === i ? 'rotate-180' : ''}`}>
                    ⌄
                  </span>
                </button>
                <div 
                ...
                >
                  <p className="px-5 pb-5 pt-0 text-[15px] text-tx-m leading-[1.65]">
                    {faq.a}
                  </p>
                </div>
              </div>
        ...
        {REVIEWS.map((r, i) => (
          <Reveal key={i} delay={i * 0.1}>
            <div className="bg-white rounded-[20px] p-6 lg:p-7 shadow-card h-full flex flex-col justify-between">
               <div>
                  <div className="text-[19px] mb-3 leading-none">{r.stars}</div>
                  <p className="text-[16px] leading-[1.65] text-tx-m mb-5">{r.text}</p>
               </div>
               <div className="flex items-center gap-3">
                 <div>
                   <div className="text-[15px] font-extrabold">{r.name}</div>
                   <div className="text-[13px] text-tx-m">{r.age}</div>
                 </div>
               </div>
            </div>
          </Reveal>
